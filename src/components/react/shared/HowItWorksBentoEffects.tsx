...
export interface HowItWorksBentoEffectsProps {
  enableStars?: boolean;
...
const HowItWorksBentoEffects: React.FC<HowItWorksBentoEffectsProps> = ({
  enableStars = true,
  ...
  useEffect(() => {
    ...
    cards.forEach((card) => {
      ...
      if (enableBorderGlow) {
        element.classList.add('howitworks-card--border-glow');
      }
    ...
    if (enableSpotlight) {
      ...
      spotlight.className = 'howitworks-global-spotlight';
      ...
      const handleMouseLeave = () => {
        ...
        document.querySelectorAll('.howitworks-card').forEach((card) => {
          (card as HTMLElement).style.setProperty('--glow-intensity', '0');
        });
...
export default HowItWorksBentoEffects;
