...
const ReferenceBentoEffects: React.FC<BentoEffectsProps> = ({
  enableStars = true,
  ...
  useEffect(() => {
    ...
    cards.forEach((card) => {
      ...
      if (enableBorderGlow) {
        element.classList.add('reference-card--border-glow');
      }
    ...
    if (enableSpotlight) {
      ...
      spotlight.className = 'reference-global-spotlight';
    ...
    <>
      <style>
        {`
          ...
          .reference-card--border-glow::after {
            content: '';
            position: absolute;
            inset: 0;
            padding: 6px;
          ...
          .reference-card--border-glow:hover::after {
            opacity: 1;
          }
...
export default ReferenceBentoEffects;
